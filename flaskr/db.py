...
# Python functions that will run "schema.sql" SQL commands to the db.py file:
...
@click.command('init-db')  # will add command in flask [init-db] --> $ flask --app flaskr/ init-db

def init_db_command():
    # call init_db() --> clear the existing data and create new tables
    init_db()
...
# Import and call this function from the factory(__init__.py). Place the new code at the end of the factory function before returning the app.